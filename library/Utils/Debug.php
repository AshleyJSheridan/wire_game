 ...
 * @category  Library
 * @package   Misc
 * @copyright Copyright (c) 2011 Andris Causs (http://codeaid.net)
 */
class Utils_Debug
{
    ...
    public static function dump ($variable, $caption = null)
    {
        ...
        echo $output;
        // print self::colorize($output);
    }

    /**
     * Colorize the specified var_dump output
     *
     * @param $output string
     *            Output of var_dump to colorize
     * @param $caption string
     *            Caption of the dump
     * @return string
     */
    public static function colorize ($output, $caption = null)
    {
        $maps = array('string' => '/(string\((?P<length>\d+)\)) (?P<value>\"(?<!\\\).*\")/i', 
        ...
        'bool' => '/bool\((?P<value>true|false)\)/', 'float' => '/float\((?P<value>[0-9\.]+)\)/', 
    ...
    {
        return '<span style="color: #0000FF;">bool</span>(<span style="color: #0000FF;">' . $matches['value'] .
         '</span>)';
    }
    ...
    {
        return '<span style="color: #0000FF;">float</span>(<span style="color: #1287DB;">' . $matches['value'] .
         '</span>)';
    }
    ...
    {
        return '<span style="color: #0000FF;">object</span>(<span style="color: #4D5D94;">' . $matches['class'] .
         '</span>)#' . $matches['id'] . ' (<span style="color: #1287DB;">' . $matches['count'] . '</span>)';
    }
    ...
    private static function _getContainerCss ()
    {
        ...
        array('background-color' => '#d6ffef', 'border' => '1px solid #bbbbbb', 'border-radius' => '4px', 
        '-moz-border-radius' => '4px', '-webkit-border-radius' => '4px', 'font-size' => '12px', 
        'line-height' => '1.4em', 'margin' => '30px', 'padding' => '7px'));
    }
    ...
    private static function _getHeaderCss ()
    {
        ...
        return self::_arrayToCss(
        array('border-bottom' => '1px solid #BBBBBB', 'font-size' => '18px', 'font-weight' => 'bold', 
        'margin' => '0 0 10px 0', 'padding' => '3px 0 10px 0'));
    }
